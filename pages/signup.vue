  ...
  <div class="addUserErea">
    ...
    <ValidationObserver v-slot="{ invalid }">
      <form class="addUserForm" @submit.prevent="signup">
        <ValidationProvider v-slot="{ errors }" rules="max:20|required|name" name="名前">
          <input class="nonSubmit" type="text" placeholder="名前" v-model="name">
        ...
        <ValidationProvider v-slot="{ errors }" rules="tel|required" name="電話番号">
          <input class="nonSubmit" type="tel" placeholder="電話番号" v-model="tel">
          <p v-show="errors.length" class="danger">
            {{ errors[0] }}
          </p>
        </ValidationProvider>
        ...
        <ValidationObserver>
          ...
          <ValidationProvider v-slot="{ errors }" rules="required|min:6|password:@パスワード" name="パスワード（再入力）">
            <input class="nonSubmit" type="password" placeholder="パスワード（再入力）" v-model="passwordConfirm">
            <p v-show="errors.length" class="danger">
              {{ errors[0] }}
            </p>
          </ValidationProvider>
        ...
        <input type="submit" value="登録" class="cursor-pointer" :disabled="invalid">
...
export default defineComponent({
  setup(){
    ...
    const signup = (({})=>{
      firebase 
        ...
        .then(res => {
          ...
          $axios.post("/api/v1/customers", { customer: newCustomer })
    ...
    return{ email, name, tel, password, passwordConfirm, show1, show2, error, signup }
  }
})
</script>
